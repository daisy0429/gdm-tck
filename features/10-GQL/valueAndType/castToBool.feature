...
Feature: cast类型转换函数-toBool
  ...
  Scenario Outline: castToBool-negative-cases
    ...
    Examples:
      ...
      | LET x = CAST(-12.3 AS BOOLEAN) RETURN x; | unsupported type in ApproximateNumericType.CastTo |
      | LET x = CAST(123.45 AS BOOLEAN) RETURN x; | unsupported type in ApproximateNumericType.CastTo |
      | LET x = CAST(DATETIME("2024-01-01T12:00:00") AS BOOLEAN) RETURN x; | unsupported type in TemporalType.CastTo |