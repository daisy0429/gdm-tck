...
# todo bug： 配置文件严格schema模式下，以下所有测试表现和非严格模式一样。 类型转换成功了（底层真实问题：导入仍然是创建的非严格模式的图）
# fixme script 严格模式语法提供/修复后，手动校验并更新测例：考虑是否能够和非严格模式共用测试数据
# ./gdm-admin -u admin -p admin123 --url http://127.0.0.1:9830 catalog graph create -g strictGraph -n "My strictGraph" --strict
#
@admin @import
