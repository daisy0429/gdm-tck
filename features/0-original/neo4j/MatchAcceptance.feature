...
Feature: MatchAcceptance
  ...
  Scenario: [1] Filter on path nodes
  ...
  Scenario: [3] Should allow AND and OR with equality predicates
    Given an empty graph
    And having executed:
      """
  ...
  Scenario: [4] Should allow AND and OR with inequality predicates
    Given an empty graph
    And having executed:
      """
      UNWIND range(1, 100) AS x CREATE (:User {prop1: x, prop2: x})
  ...
  Scenario: [5] Should allow AND and OR with STARTS WITH predicates
    Given an empty graph
    And having executed:
      """
  ...
  Scenario: [6] Should allow AND and OR with regex predicates
    Given an empty graph
    And having executed:
      """
      UNWIND range(1, 100) AS x CREATE (:User {prop1: x+'_val', prop2: x+'_val'})
  ...
  Scenario: [7] Should allow OR with regex predicates
    Given an empty graph
    And having executed:
      """
  ...
  Scenario: [8] difficult to plan query number 1
    Given an empty graph
    And having executed:
      """
      CREATE (:A {foo: 42})-[:T]->(),
  ...
  Scenario: [9] difficult to plan query number 2
    Given an empty graph
    And having executed:
      """
      CREATE (ts: TS),
  ...
  Scenario: [10] difficult to plan query number 3
    Given an empty graph
    And having executed:
    """
    CREATE (:A {foo: 42})-[:T]->(),
  ...
  Scenario: [11] Match on multiple labels
    Given an empty graph
    And having executed:
      """
      CREATE (:A:B), (:A:C), (:B:C)
  ...
  Scenario: [12] Match on multiple labels with OR
    Given an empty graph
    And having executed:
      """
      CREATE (:A:B), (:A:C), (:B:C)
  ...
  Scenario: [13] Handle filtering with empty properties map
    Given an empty graph
    And having executed:
      """
  ...
  Scenario: [14] Variable length path with both sides already bound
    Given an empty graph
    And having executed:
      """
  ...
  Scenario: [17] Should handle simple IS NOT NULL on node property when node is null
    Given an empty graph
    And having executed:
      """
      CREATE (:LBL1)
  ...
  Scenario: [18] Should handle complex IS NOT NULL on node property when node is null
    Given an empty graph
    And having executed:
      """
  ...
  Scenario: [19] equality with boolean lists
    Given an empty graph
    And having executed:
      """
      CREATE ({prop: [false]})
  ...
  Scenario: [20] loops with relationship type
    Given an empty graph
    And having executed:
      """
      CREATE (x:V)-[:R]->(x)
  ...
  Scenario: [21] Expand on a node stored in a map
    Given an empty graph
    And having executed:
      """
