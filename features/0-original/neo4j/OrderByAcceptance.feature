...
Feature: OrderByAcceptance
  ...
  Scenario: [1] ORDER BY nodes should return null results last in ascending order
  ...
  Scenario: [2] ORDER BY relationships should return null results last in ascending order
    Given an empty graph
    And having executed:
      """
      CREATE (:A)-[:REL]->(:B),
  ...
  Scenario: [3] ORDER BY with unwind primitive integer
    Given any graph
    When executing query:
      """
      WITH [4, 3, 1, 2] AS lst
  ...
  Scenario: [4] ORDER BY two node properties
    Given an empty graph
    And having executed:
      """
      CREATE (:L {a: 3, b: "a"}),
  ...
  Scenario: [5] ORDER BY two node properties with LIMIT
    Given an empty graph
    And having executed:
      """
      CREATE (:L {a: 3, b: "a"}),
  ...
  Scenario: [6] Ordering is well defined across all types, ascending
    Given an empty graph
    And having executed:
      """
      CREATE (:A)-[:T]->()
  ...
  Scenario: [7] Ordering is well defined across all types, descending
    Given an empty graph
    And having executed:
      """
      CREATE (:A)-[:T]->()
  ...
  Scenario: [8] Ordering for lists, ascending
    Given an empty graph
    And having executed:
      """
  ...
  Scenario: [9] Ordering for lists, descending
    Given an empty graph
    And having executed:
      """
      UNWIND [1, true, 'foo'] AS element
  ...
  Scenario: [10] ORDER BY node problem
    Given an empty graph
    And having executed:
      """
      CREATE (:A)-[:REL]->(:B)
  ...
  Scenario: [11] ORDER BY on nested map missing property
    Given an empty graph
    When executing query:
      """
      WITH {key: 'foo'} AS m
  ...
  Scenario: [13] ORDER BY which needs to evaluate expression in WITH
    Given an empty graph
    And having executed:
      """
  ...
  Scenario: [14] ORDER BY which needs to evaluate expression in RETURN
    Given an empty graph
    And having executed:
      """
      CREATE (:A {age: 10, name: 'A'})
