...
Feature: OptionalMatchAcceptance
  ...
  Scenario: [3] optional equality with boolean lists
    Given an empty graph
    And having executed:
      """
      CREATE ({prop: [false]})
  ...
  Scenario: [4] optional match with distinct
    Given an empty graph
    And having executed:
      """
      CREATE (:A)-[:Y]->(:B)-[:X]->(:B)
  ...
  Scenario: [5] optional match with OR where clause
    Given an empty graph
    And having executed:
    # Setup: (a)<->(b)->(c)
    """
  ...
  Scenario: [6] optional match followed by match with repeated null relationship
    Given an empty graph
    When executing query:
      """
  ...
  Scenario: [7] double optional match with repeated null relationship
    Given an empty graph
    When executing query:
      """
       OPTIONAL MATCH (n)-[r]->(m)
  ...
  Scenario: [8] optional match followed by aggregation and map-projection
    Given an empty graph
    When executing query:
      """
       OPTIONAL MATCH (n)-->(m)
