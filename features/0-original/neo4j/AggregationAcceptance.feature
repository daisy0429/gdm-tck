...
Feature: AggregationAcceptance
  ...
  Scenario: [1] Using a optional match after aggregation and before an aggregation
    And having executed:
      """
  ...
  Scenario: [2] Multiple aggregations should work
    And having executed:
      """
  ...
  Scenario: [3] Distinct should work with multiple equal grouping keys and only one different
    And having executed:
      """
  ...
  Scenario: [4] percentileDisc on empty data should return null
    When executing query:
      """
  ...
  Scenario: [5] optional match followed by aggregation
    Given an empty graph
    When executing query:
    """
   OPTIONAL MATCH (n)
  ...
  Scenario: [6] aggregation on function with null argument
    Given an empty graph
    When executing query:
    """
