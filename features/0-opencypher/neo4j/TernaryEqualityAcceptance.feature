...
Feature: TernaryEqualityAcceptance
  ...
  Scenario Outline: [1] equality between different types should yield false
    And parameters are:
      | lhs | <lhs> |
      | rhs | <rhs> |
  ...
  Scenario: [2] List equality [1, 2, 3] and [1, null, 3]
    And parameters are:
      | lhs | [1, 2, 3]   |
      | rhs | [1, null, 3] |
    When executing query:
  ...
  Scenario: [3] List equality [1, 2, 3] and [1, null, 4]
    And parameters are:
      | lhs | [1, 2, 3]   |
      | rhs | [1, null, 4] |
    When executing query:
  ...
  Scenario: [4] List equality [1, 2, 3] and [1, 'two', 3]
    And parameters are:
      | lhs | [1, 2, 3]   |
      | rhs | [1, 'two', 4] |
    When executing query:
  ...
  Scenario: [5] Map equality {k: 42} and {k: null}
    And parameters are:
      | lhs | {k: 42} |
      | rhs | {k: null}|
    When executing query:
  ...
  Scenario: [6] Map equality {k1: 42} and {k2: null}
    And parameters are:
      | lhs | {k1: 42} |
      | rhs | {k2: null}|
    When executing query:
  ...
  @ignore
  Scenario: [7] Map equality {k: 42} and {null: 42}
    And parameters are:
      | lhs | {k: 42} |
      | rhs | {null: 42}|
    When executing query:
    ...
    @ignore
  Scenario: [8] Map equality {k: 42} and {null: 43}
    And parameters are:
      | lhs | {k: 42} |
      | rhs | {null: 43}|
    When executing query:
  ...
  Scenario: [9] Map equality {k1: 42, k2: 43, k3: 44} and {k1: 42, k2: null, k3: 'fortyfour'}
    And parameters are:
  ...
  Scenario Outline: [10] Spatial equality test
    When executing query:
      """
  ...
  Scenario: [11] Duration equality test of equal durations
  ...
  Scenario: [12] Duration equality test of different durations
    Given an empty graph
    And having executed:
      """
